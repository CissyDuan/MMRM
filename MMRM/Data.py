...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        s_ids_mask, mask_pic,pic, mask_index, tgt_id = get_maskdata(ids, mask_pad, data_t)
...
sample_num=1
def get_sample_pic(sample_weight,data_t):
...
def get_maskdata(s_ids,s_mask_pad,data_t):
    ...
    #print(sample_weight)
    mask_index, pic_ori, pic_mask=get_sample_pic(sample_weight,data_t)
    if pic_ori==None:
        mask_index, pic_ori, pic_mask = get_sample_pic(sample_weight, data_t)
    if pic_ori==None:
        mask_index, pic_ori, pic_mask = get_sample_pic(sample_weight, data_t)
...
def trans_noise(image,seed):
    ...
    image = torch.min(torch.max((torch.randn_like(image) / (1+1* torch.rand(1,device=torch.device("cuda"))) + image), torch.tensor(0,device=torch.device("cuda"))),
...
def get_pic(text,font):
    ...
    if pic_filter(image)==True:
        ...
        image=pic_mask(image,width,height)
...
def pic_mask(image,width,height):
    x=random.random()
